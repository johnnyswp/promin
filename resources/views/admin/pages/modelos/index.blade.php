    ...
    <div class="well" style="overflow: auto">
      ...
      <div class="row">
        ...
        <div class="col-md-3">
            ...
            {{Form::select('filtro', ['all'=>'Todos','linea'=>'Linea de negocio','marca'=>'Marca','modelo'=>'Modelo','tipo'=>'Tipo de producto'], $input->filtro, ['class'=>'form-control', 'onchange'=>'this.form.submit()'])}}
        ...
        <table class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{Carbon\Carbon::parse($modelo->created_at)->format('d').'-'.trans('main.'.Carbon\Carbon::parse($modelo->created_at)->format('m')).'-'.Carbon\Carbon::parse($modelo->created_at)->format('Y')}}</td>
              ...
              <a data-toggle="modal" data-target="#myModal{{$modelo->id}}" class="btn btn-danger btn-xs" alt="Eliminar"><i class="fa fa-remove"></i></a>
                            <div class="modal fade" id="myModal{{$modelo->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                              <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                    <h4 class="modal-title" id="myModalLabel">Desea eliminar este modelo?</h4>
                                  </div>
                                 
                                  <div class="modal-footer">
                                    <form id="del-form{{$modelo->id}}" action="{{ route('modelos.destroy',$modelo->id) }}" method="POST" style="display: block;">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
